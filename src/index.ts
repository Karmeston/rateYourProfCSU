...
function browseQuery(query: Record<string, string[]>) {
  if (Object.keys(query).some((key) => !['limit', 'offset', 'q'].includes(key))) return null;
  const values = { limit: 20, offset: 0, q: '' };
...
app.get('/api/courses', async (c) => {
  ...
  const { results } = await c.env.DB.prepare(
    'SELECT id, code, name, department FROM courses WHERE instr(name, ?) > 0 ORDER BY id ASC LIMIT ? OFFSET ?',
  ).bind(page.q, page.limit + 1, page.offset).all<Course>();
